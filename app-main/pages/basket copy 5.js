...
export default function Basket() {
  ...
  const renderCustomerDetails = () => {
    return (
      ...
      ${customerDetails.fullName ? '-top-3 text-xs' : 'top-2 text-base'
                }
      ...
      `}
            ...
            {/* SVG icon */}
            {errors.fullName ? (
              <ExclamationCircleIcon className="absolute right-3 top-2 h-7 w-7 text-red-600" />
            ) : customerDetails.fullName ? (
              <CheckCircleIcon className="absolute right-3 top-2 h-7 w-7 text-green-500" />
            ) : null}
          </div>
