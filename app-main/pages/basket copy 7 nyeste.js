...
export default function Basket() {
  ...
  const isValidPostalCode = (postalCode) => /^\d{4}$/.test(postalCode.trim());
  ...
  const handleInputBlur = (fieldName) => {
    ...
    // If postalCode changed and is valid, run DAWA
    if (fieldName === 'postalCode' && deliveryOption === 'pickupPoint') {
      if (isValidPostalCode(updatedDetails.postalCode || '')) {
        setLoading(true);
        debouncedValidateAddressWithDAWA();
      }
    }
  };
  ...
  const debouncedValidateAddressWithDAWA = useCallback(
    debounce(validateAddressWithDAWA, 500),
    []
  );
  ...
  useEffect(() => {
    if (deliveryOption === 'pickupPoint' && isValidPostalCode(customerDetails.postalCode || '')) {
      setLoading(true);
      debouncedValidateAddressWithDAWA();
    } else {
      setPickupPoints([]);
      ...
      <div id="shipping-and-payment">
        ...
        {deliveryOption === 'pickupPoint' && (
          <>
            ...
            {!isValidPostalCode(customerDetails.postalCode || '') ? (
              <p>Indtast venligst et gyldigt postnummer.</p>
            ) : pickupPoints.length === 0 ? (
              <p>Ingen afhentningssteder fundet for det angivne postnummer.</p>
            ) : (
              <div className="mt-2">
                <select
                  value={selectedPoint || ''}
                  onChange={(e) => handleSelectedPointChange(e.target.value)}
                  className="w-full px-3 py-2 border rounded font-semibold focus:outline-none"
                >
