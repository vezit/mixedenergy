...
const LoginForm: React.FC<LoginFormProps> = ({ redirectPath }) => {
  ...
  useEffect(() => {
    if (redirectPath === '/admin') {
      const adminEmail: string | undefined = process.env.NEXT_PUBLIC_ADMIN_ACCOUNT_NAME;
      const adminPassword: string | undefined = process.env.NEXT_PUBLIC_ADMIN_ACCOUNT_PASSWORD;

      if (adminEmail) setEmail(adminEmail);
      if (adminPassword) setPassword(adminPassword);
    } else if (redirectPath === '/management') {
      const managementEmail: string | undefined = process.env.NEXT_PUBLIC_MANAGEMENT_ACCOUNT_NAME;
      const managementPassword: string | undefined =
        process.env.NEXT_PUBLIC_MANAGEMENT_ACCOUNT_PASSWORD;
  ...
  const handleLogin = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        }, 1000); // Wait 100ms (or 1000ms) before navigating
      } else {
