...
// console.log(process.env.FIREBASE_ADMIN_KEY); // Debug: Check if this is correctly defined
...
async function populateDrinks() {
  ...
  for (const drink of drinksData) {
    ...
    const drinkDoc = {
      ...drink,
      price: parseFloat(drink.price.replace(' kr', '').replace(',', '.')),
      packageQuantity: parseInt(drink.packageQuantity),
      brand: brand,
    };

    await db.collection('drinks').doc(drink.id).set(drinkDoc);
